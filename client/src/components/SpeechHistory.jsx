...
export function SpeechHistory({
  ...
  const visible = useMemo(() => {
    let msgs = tab === "pinned"
      ? history.filter((m) => favorites.has(m.id))
      : history;
      ...
      {!collapsed && (
        <>
          ...
          <div className="flex-shrink-0 border-b border-neutral-200 px-3 py-2 dark:border-neutral-700">
            <label htmlFor="vf-search" className="sr-only">Search history</label>
            <input
              id="vf-search"
              type="search"
              value={search}
          ...
          {/* Tabs */}
          <div className="flex flex-shrink-0 gap-1 border-b border-neutral-200 px-3 pt-2 dark:border-neutral-700" role="tablist">
            {[
              { key: "all", label: "All" },
...
function EmptyState({ tab, hasSearch }) {
  ...
  if (tab === "pinned") {
    return (
      <div className="flex flex-col items-center py-10 text-center text-sm text-neutral-400">
        <span className="mb-2 text-3xl">⭐</span>
        <p>No pinned messages yet.<br />Star a message to pin it here.</p>
      </div>
    );
  }
  return (
    <div className="flex flex-col items-center py-10 text-center text-sm text-neutral-400">
      ...
      <p>No history yet.<br />Speak a message to get started.</p>
    </div>
  );
}
